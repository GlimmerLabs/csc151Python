...
#                it does not exist.
#Returns True and False for increased readability. Returns dbus.32(1)
#and dbus.32(0) by default.
...
def drawing_group(*drawings):
    a = 0
    for x in drawings:
        if(drawing_type(drawings[a]) == 'group'):
            a = a + 1
        elif (drawing_validate(drawings[a])):
            a = a + 1
        else:
...
#Preconditions:
#Postconditions:
def drawing_group_unpack(drawings):
    length = len(drawings)
    final = []
    if length < 0:
        return final
        ...
        i = 0
        if drawing_type(drawings[i] == "group"):
            drawing_group_unpack(drawing_members(drawings[i]))
            i += 1
        else:
            final.append(drawings[i])
            i += 1
        drawing_group_unpack[i]       
    #return 
...
def drawing_members(drawing):
    if(drawing_validate(drawing)):
        if (drawing_type(drawing) == "group"):
            return drawing[2:] # Drops the 'drawing' 'group'
        else:
            return drawing
    else:
...
#Produces: result, a boolean
#HEY FIGURE OUT A WAY TO CHECK IF TYPE IS INT/FLOAT/YOU KNOW WHAT
#i'M TALKING ABOUT!!!!
